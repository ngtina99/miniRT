...
/*   Updated: 2024/11/16 01:30:33 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	open_rt(int argc, char **argv)
{
	...
	file_name = argv[1];
	if (ft_strlen(file_name) < 3 || ft_strcmp(file_name + ft_strlen(file_name) - 3, ".rt") != 0)
	{
	...
	if (fd == -1)
	{
		printf(RED "Error: input is not an existing .rt file path or the file can't be opened\n" RESET);
		return (-1);
	}
	return (fd);
}
